...
const questions = [
  ...
  { 
    id: "number", 
    question: "数値を扱う",
    consideration: "小数点以下の桁数と丸め処理, 0が表示されているか, 0をfalseとして判定していないか, nullまたは未定義値に対して数値メソッド（toFixed等）を呼び出していないか"
  },
  { 
    id: "string", 
  ...
  { 
    ...
    { 
    id: "table", 
    question: "画面に表（テーブル）がある",
    consideration: "右寄せ、左寄せは適切か, ヘッダーは固定するかしないか, 列幅の調整はできているか, ソート機能の有無とその挙動は適切か"
  },
  { 
    id: "required", 
    question: "必須項目がある",
  ...
  { 
    id: "calc", 
    question: "計算処理がある",
    consideration: "浮動小数点演算の誤差対策, 除算時のゼロ除算エラー（0で割り算してないか）, nullまたは未定義値に対して数値メソッド（toFixed等）を呼び出していないか, DBがfloat型の場合、丸め誤差が発生する可能性があることを考慮しているか"
  },
  { 
    id: "file", 
